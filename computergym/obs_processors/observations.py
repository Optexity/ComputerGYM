...
def _pre_extract(
    ...
    js_frame_mark_elements = pkgutil.get_data(
        __name__, "javascript/frame_mark_elements.js"
    ).decode("utf-8")
    ...
    def mark_frames_recursive(frame: playwright.sync_api.Frame, frame_bid: str):
        ...
        for child_frame in frame.child_frames:
            ...
            if sandbox_attr is not None and "allow-scripts" not in sandbox_attr.split():
                continue
            child_frame_bid = child_frame_elem.get_attribute(BROWSERGYM_ID_ATTRIBUTE)
    ...
    for frame in page.frames:
        ...
        try:
            if not frame == page.main_frame:
                ...
                if (
                    sandbox_attr is not None
                    and "allow-scripts" not in sandbox_attr.split()
                ):
                    continue
                # deal with frames without a BID
                bid = frame.frame_element().get_attribute(BROWSERGYM_ID_ATTRIBUTE)
                if bid is None:
                    continue
            ...
            if any(
                msg in str(e)
                for msg in ("Frame was detached", "Frame has been detached")
            ):
                pass
            else:
                raise e
    ...
    while docs_to_process:
        ...
        if parent:
            ...
            if node_layout_idx >= 0:
                ...
                parent_node_abs_x = (
                    doc_properties[parent_doc]["abs_pos"]["x"] + node_bounds[0]
                )
                parent_node_abs_y = (
                    doc_properties[parent_doc]["abs_pos"]["y"] + node_bounds[1]
                )
            else:
                parent_node_abs_x = 0
                parent_node_abs_y = 0
        ...
        for node_idx, bounds, client_rect in zip(
            ...
            if not client_rect:
                ...
                doc_properties[doc]["nodes"][node_idx]["bbox"][0] += doc_properties[
                    doc
                ]["abs_pos"]["x"]
                doc_properties[doc]["nodes"][node_idx]["bbox"][1] += doc_properties[
                    doc
                ]["abs_pos"]["y"]
        ...
        for node in doc_properties[doc]["nodes"]:
            ...
            if bid:
                if bid in extra_properties:
                    logger.warning(
                        f"duplicate {BROWSERGYM_ID_ATTRIBUTE}={repr(bid)} attribute detected"
                    )
